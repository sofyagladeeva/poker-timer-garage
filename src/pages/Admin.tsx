...
function buildGiftNotifyText(
  ...
  if (type === 'custom') {
    return `🎁 Подарок от Garage: ${comment ?? 'сюрприз'}`;
  }

  const typeLabel =
    type === 'free_entry' ? 'бесплатный вход'
    : type === 'free_rebuy' ? 'бесплатный ребай'
    : 'бесплатный аддон';

  let validity = '';
  if (validCondition === 'next_tournament') {
    validity = 'на следующем турнире';
  } else if (validCondition === 'specific_tournament' && validTournamentTitle) {
    validity = `на турнире «${validTournamentTitle}»`;
  } else if ((validCondition === 'until_date' || validCondition === 'next_month') && validUntil) {
    const date = new Date(validUntil).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' });
    validity = `до ${date}`;
  }

  const lines = [`🎁 Garage дарит тебе ${typeLabel}`];
  if (validity) lines.push(`Действует: ${validity}`);
  return lines.join('\n');
}
...
export function Admin() {
      ...
      if (winner?.telegramId != null) {
        try {
          ...
          const notifyText = gameState.tournamentTitle
            ? `🏆 Чемпион\nТы взял «${gameState.tournamentTitle}» — и это не случайность. Держи бесплатный вход на следующий турнир Garage. Увидимся за столом!`
            : '🏆 Чемпион\nТы выиграл этот турнир — и это не случайность. Держи бесплатный вход на следующий Garage. Увидимся за столом!';
  ...
  const handleCreateGiftFromPrizesTab = useCallback(async () => {
    ...
    try {
      ...
      const validUntil = giftDraft.validCondition === 'until_date' ? (giftDraft.validUntil || null)
        ...
        : null;
      const comment = giftDraft.type === 'custom' ? (giftDraft.comment || null) : null;
      const validTournamentTitle = giftDraft.validCondition === 'specific_tournament' ? (giftDraft.validTournamentTitle || null) : null;
      const gift = await createGift({
  ...
  const handleIssueMonthlyPrize = async (
    ...
    try {
      ...
      const monthRaw = new Date(`${month}-01`).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' });
      const monthLabel = monthRaw.charAt(0).toUpperCase() + monthRaw.slice(1);
      const notifyText = source === 'auto_monthly_rating'
        ? `📊 ${monthLabel} закрыт — и ты на вершине рейтинга. Garage такое не забывает: держи бесплатный вход на следующий месяц. Не сбавляй!`
        : `🎯 ${monthLabel} — ты охотился жёстче всех. Баунти-трон твой по праву. Бесплатный вход на следующий месяц, приходи и продолжай охоту!`;
  ...
  const handleIssueDoubleMonthlyPrize = async (leader: PlayerAggregate, month: string) => {
    ...
    try {
      ...
      const monthRaw = new Date(`${month}-01`).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' });
      const monthLabel = monthRaw.charAt(0).toUpperCase() + monthRaw.slice(1);
      const notifyText = `💥 ${monthLabel} — чистое доминирование. Ты забрал и рейтинг, и баунти. Garage отвечает тем же: бесплатный вход + аддон на следующий месяц. Заслужено на все сто!`;
      const notifyResult = await sendPlayerNotification(leader.telegramId, notifyText);
      if (notifyResult.ok && !notifyResult.skipped) {
