...
const RESULTS_URL_TEMPLATE = import.meta.env.VITE_BOT_TOURNAMENT_RESULTS_URL_TEMPLATE || `${BOT_API}/api/games/{id}/results`;
...
export interface ImportedTournamentRoster {
  players: ImportedTournamentPlayer[];
  waitlist: ImportedTournamentPlayer[];
  url: string;
}
...
function normalizeImportedPlayer(
  ...
  const username = (
    toNullableString(source.username) ||
...
function normalizeRosterResponse(raw: unknown): Pick<ImportedTournamentRoster, 'players' | 'waitlist'> {
